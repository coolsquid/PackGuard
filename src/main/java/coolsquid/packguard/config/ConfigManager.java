...
public class ConfigManager {
	...
	public static void loadConfig() throws IOException {
		...
		} else if (expectedModsStrings.length % 2 == 0) {
			for (int i = 0; i < expectedModsStrings.length; i++) {
				if (expectedModsStrings[i].equals("FML") && expectedModsStrings.length > i + 1 && expectedModsStrings[i + 1].matches("^\\d+(\\.\\d+)*$")) {
					String[] newExpectedModsStrings = new String[expectedModsStrings.length / 2];
					for (int o = 0; o < expectedModsStrings.length; o += 2) {
						String a = expectedModsStrings[o];
						String b = expectedModsStrings[o + 1];
						newExpectedModsStrings[o / 2] = a + ";[" + b + "]";
					}
					CONFIG.get("mods", "expectedMods", new String[0]).set(newExpectedModsStrings);
					expectedModsStrings = newExpectedModsStrings;
					break;
				}
			}
		}
		for (String s : expectedModsStrings) {
			...
			if (a.length == 2) {
				...
				throw new RuntimeException("Could not parse mod version: " + s);
			}
		}
